...
import {Card, CardHeader, CardBody, Button, CardProps, Chip, Link, Tooltip} from '@nextui-org/react'
...
export function FinancialServiceSetting(props: CardProps) {
    ...
    return (
        <>
                ...
                <CardBody className="flex flex-col gap-2">
                    ...
                    <CellWrapper>
                        <div>
                            ...
                            <p className="text-small text-default-500">
                                Enter your own OpenBB token to get access to all of your personal data vendors compiled
                                by OpenBB.&nbsp;
                                <Tooltip
                                    content={
                            ...
                            <div className="flex mr-2 flex-col items-end">
                                {
                                    profile?.openbb_token && profile?.openbb_token_status ?
                                        <p className="text-small text-success">Activated</p> : null
                                }

                                {
                                    profile?.openbb_token && !profile?.openbb_token_status ?
                                        <p className="text-small text-warning">Not Activated</p> : null
                                }

                                {
                                    !profile?.openbb_token && !profile?.openbb_token_status ?
                                        <p className="text-small text-danger">Not Configured</p> : null
                                }
                            </div>
                            <Button
                    ...
                    <CellWrapper className="bg-content1">
                        <div>
                            <p>InteractiveBroker API Key <Chip radius="sm" size="sm" color="warning" className="ml-2">Coming
                                Soon</Chip></p>
                            <p className="text-small text-default-500">
                                Enter your Interactive Broker token to view live account data and start implementing
                                strategies
                            </p>
                        </div>
                        <div className="flex w-full flex-wrap items-center justify-end gap-6 sm:w-auto sm:flex-nowrap">
            ...
            {/* Phone Modal */}
            <OpenBBTokenModal toast={toast} openBBAction={openBB} setOpenBBAction={setOpenBB}/>
